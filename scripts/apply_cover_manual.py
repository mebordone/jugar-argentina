#!/usr/bin/env python3
"""Aplica portadas investigadas manualmente al catalogo."""
...
RESEARCH_PATH = DATA / "cover_manual_research.json"


def apply_manual_covers(games: list[dict], entries: list[dict]) -> tuple[int, int]:
    ...
    for entry in entries:
        ...
        if imagenes.get("portada"):
            ...
            entry["omitido"] = "ya_tiene_portada"
            skipped += 1
            continue

        portada = entry.get("portada_propuesta")
        if not isinstance(portada, str) or not portada.startswith(("http://", "https://")):
            skipped += 1
            continue

        imagenes["portada"] = portada
...
def main() -> int:
    ...
    applied, skipped = apply_manual_covers(games, entries)
    ...
    print(f"Portadas manuales aplicadas: {applied}")
